...
using namespace CCNDTLib;

DgmOctreeNDTReferenceCloud::DgmOctreeNDTReferenceCloud(DgmOctreeNDT::NeighboursSet* associatedSet, unsigned size/*=0*/)
	: m_globalIterator(0)
	, m_validBB(false)
	, m_set(associatedSet)
...
void DgmOctreeNDTReferenceCloud::computeBB()
{
	//empty cloud?!
	unsigned count = size();
	if (count)
...
void DgmOctreeNDTReferenceCloud::getBoundingBox(CCVector3& bbMin, CCVector3& bbMax)
{
	if (!m_validBB)
		computeBB();
...
void DgmOctreeNDTReferenceCloud::forEach(genericPointAction& action)
{
	unsigned count = size();
