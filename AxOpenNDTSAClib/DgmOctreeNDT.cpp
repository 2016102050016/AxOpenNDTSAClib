...
namespace CCNDTLib
{
bool DgmOctreeNDT::MultiThreadSupport()
{
...
DgmOctreeNDT::DgmOctreeNDT(GenericIndexedCloudPersist* cloud)
...
int DgmOctreeNDT::build(GenericProgressCallback* progressCb)
{
...
int DgmOctreeNDT::build(	const CCVector3& octreeMin,
...
int DgmOctreeNDT::genericBuild(GenericProgressCallback* progressCb)
{
	...
	if (m_numberOfProjectedPoints > 1)
	{
		...
		unsigned char currentBitDec = GET_NDT_BIT_SHIFT(currentLevel);
...
void DgmOctreeNDT::updateMinAndMaxTables()
{
...
void DgmOctreeNDT::updateCellSizeTable()
{
...
void DgmOctreeNDT::updateCellCountTable()
{
...
void DgmOctreeNDT::computeCellsStatistics(unsigned char level)
{
...
struct MonoDimensionalCellCodes
{
	...
	**/
	static const int VALUE_COUNT = OCTREE_LENGTH(CCNDTLib::DgmOctreeNDT::MAX_OCTREE_LEVEL);
	...
	MonoDimensionalCellCodes()
	{
		...
		for (int value = 0; value < VALUE_COUNT; ++value)
		{
			...
			CCNDTLib::DgmOctreeNDT::OctreeCellCodeType code = 0;
			for (unsigned char k=0; k<CCNDTLib::DgmOctreeNDT::MAX_OCTREE_LEVEL; k++)
			{
				mask >>= 1;
				code <<= 3;
				if (value & mask)
	...
	CCNDTLib::DgmOctreeNDT::OctreeCellCodeType values[VALUE_COUNT];
...
DgmOctreeNDT::OctreeCellCodeType DgmOctreeNDT::generateTruncatedCellCode(const Tuple3i& cellPos, unsigned char level) const
{
	...
	return	(	 PRE_COMPUTED_POS_CODES.values[cellPos.x << dec]
			...
			) >> GET_NDT_BIT_SHIFT(level);
}

#ifndef OCTREE_CODES_64_BITS
DgmOctreeNDT::OctreeCellCodeType DgmOctreeNDT::generateTruncatedCellCode(const Tuple3s& cellPos, unsigned char level) const
{
	assert(		cellPos.x >= 0 && cellPos.x < MonoDimensionalCellCodes::VALUE_COUNT
			&&	cellPos.y >= 0 && cellPos.y < MonoDimensionalCellCodes::VALUE_COUNT
			&&	cellPos.z >= 0 && cellPos.z < MonoDimensionalCellCodes::VALUE_COUNT );
...
static inline DgmOctreeNDT::OctreeCellCodeType GenerateCellCodeForDim(int pos)
{
	return PRE_COMPUTED_POS_CODES.values[pos];
}

void DgmOctreeNDT::getBoundingBox(CCVector3& bbMin, CCVector3& bbMax) const
{
	bbMin = m_dimMin;
	bbMax = m_dimMax;
}

void DgmOctreeNDT::getCellPos(OctreeCellCodeType code, unsigned char level, Tuple3i& cellPos, bool isCodeTruncated) const
{
	//binary shift for cell code truncation
	if (!isCodeTruncated)
		code >>= GET_NDT_BIT_SHIFT(level);
...
void DgmOctreeNDT::computeCellLimits(OctreeCellCodeType code, unsigned char level, CCVector3& cellMin, CCVector3& cellMax, bool isCodeTruncated) const
{
	Tuple3i cellPos;
...
bool DgmOctreeNDT::getPointsInCell(OctreeCellCodeType cellCode,
								unsigned char level,
...
unsigned DgmOctreeNDT::getCellIndex(OctreeCellCodeType truncatedCellCode, unsigned char bitDec) const
{
...
unsigned DgmOctreeNDT::getCellIndex(OctreeCellCodeType truncatedCellCode, unsigned char bitDec, unsigned begin, unsigned end) const
{
	assert(truncatedCellCode != INVALID_CELL_CODE);
...
unsigned DgmOctreeNDT::findPointNeighbourhood(const CCVector3* queryPoint,
											ReferenceCloud* Yk,
...
void DgmOctreeNDT::getCellDistanceFromBorders(	const Tuple3i& cellPos,
											unsigned char level,
											int* cellDists) const
{
...
void DgmOctreeNDT::getNeighborCellsAround(const Tuple3i& cellPos,
										...
										unsigned char level) const
{
	...
	//binary shift for cell code truncation
	const unsigned char bitDec = GET_NDT_BIT_SHIFT(level);
...
void DgmOctreeNDT::getPointsInNeighbourCellsAround(NearestNeighboursSearchStruct &nNSS,
												int neighbourhoodLength,
												bool getOnlyPointsWithValidScalar/*=false*/) const
{
	...
	//binary shift for cell code truncation
	const unsigned char bitDec = GET_NDT_BIT_SHIFT(nNSS.level);
...
void DgmOctreeNDT::getPointsInNeighbourCellsAround(NearestNeighboursSphericalSearchStruct &nNSS,
												...
												int maxNeighbourhoodLength) const
{
	...
	//binary shift for cell code truncation
	unsigned char bitDec = GET_NDT_BIT_SHIFT(nNSS.level);
...
double DgmOctreeNDT::findTheNearestNeighborStartingFromCell(NearestNeighboursSearchStruct &nNSS) const
{
	//binary shift for cell code truncation
	unsigned char bitDec = GET_NDT_BIT_SHIFT(nNSS.level);
	...
	while (true)
	{
		...
		DgmOctreeNDT::cellIndexesContainer::const_iterator q;
...
unsigned DgmOctreeNDT::findNearestNeighborsStartingFromCell(	NearestNeighboursSearchStruct &nNSS,
															bool getOnlyPointsWithValidScalar/*=false*/) const
{
	//binary shift for cell code truncation
	unsigned char bitDec = GET_NDT_BIT_SHIFT(nNSS.level);
...
int DgmOctreeNDT::getPointsInSphericalNeighbourhood(	const CCVector3& sphereCenter,
...
size_t DgmOctreeNDT::getPointsInBoxNeighbourhood(BoxNeighbourhood& params) const
{
	//cell size
	...
	//binary shift for cell code truncation
	unsigned char bitDec = GET_NDT_BIT_SHIFT(params.level);
...
size_t DgmOctreeNDT::getPointsInCylindricalNeighbourhood(CylindricalNeighbourhood& params) const
{
	//cell size
	const PointCoordinateType& cs = getCellSize(params.level);
	PointCoordinateType halfCellSize = cs/2;
...
size_t DgmOctreeNDT::getPointsInCylindricalNeighbourhoodProgressive(ProgressiveCylindricalNeighbourhood& params) const
{
	//cell size
	const PointCoordinateType& cs = getCellSize(params.level);
	PointCoordinateType halfCellSize = cs/2;
...
int DgmOctreeNDT::findNeighborsInASphereStartingFromCell(NearestNeighboursSphericalSearchStruct &nNSS, double radius, bool sortValues) const
{
#ifdef OCTREE_TREE_TEST
	assert(s_root);
...
unsigned char DgmOctreeNDT::findBestLevelForAGivenNeighbourhoodSizeExtraction(PointCoordinateType radius) const
{
...
unsigned char DgmOctreeNDT::findBestLevelForComparisonWithOctree(const DgmOctreeNDT* theOtherOctree) const
{
...
unsigned char DgmOctreeNDT::findBestLevelForAGivenPopulationPerCell(unsigned indicativeNumberOfPointsPerCell) const
{
...
unsigned char DgmOctreeNDT::findBestLevelForAGivenCellNumber(unsigned indicativeNumberOfCells) const
{
...
double DgmOctreeNDT::computeMeanOctreeDensity(unsigned char level) const
{
	return static_cast<double>(m_numberOfProjectedPoints)/static_cast<double>(getCellNumber(level));
}

bool DgmOctreeNDT::getCellCodesAndIndexes(unsigned char level, cellsContainer& vec, bool truncatedCodes/*=false*/) const
{
	try
	{
		//binary shift for cell code truncation
...
bool DgmOctreeNDT::getCellCodes(unsigned char level, cellCodesContainer& vec, bool truncatedCodes/*=false*/) const
{
	try
	{
		//binary shift for cell code truncation
...
ReferenceCloud* DgmOctreeNDT::getPointsInCellsWithSortedCellCodes(	cellCodesContainer& cellCodes,
																unsigned char level,
																ReferenceCloud* subset,
																bool areCodesTruncated/*=false*/) const
{
    ...
    //binary shift for cell code truncation
    unsigned char bitDec1 = GET_NDT_BIT_SHIFT(level); //shift for this octree codes
...
void DgmOctreeNDT::diff(const cellCodesContainer& codesA, const cellCodesContainer& codesB, cellCodesContainer& diffA, cellCodesContainer& diffB) const
{
...
void DgmOctreeNDT::diff(unsigned char octreeLevel, const cellsContainer &codesA, const cellsContainer &codesB, int &diffA, int &diffB, int &cellsA, int &cellsB) const
{
	...
	//binary shift for cell code truncation
	unsigned char bitDec = GET_NDT_BIT_SHIFT(octreeLevel);
...
int DgmOctreeNDT::extractCCs(unsigned char level, bool sixConnexity, GenericProgressCallback* progressCb) const
{
...
//提取连通区域
int DgmOctreeNDT::extractCCs(const cellCodesContainer& cellCodes, unsigned char level, bool sixConnexity, GenericProgressCallback* progressCb) const
{
	size_t numberOfCells = cellCodes.size();//Cell的数目
    ...
    Tuple3i indexMin, indexMax;
	{
		...
		for (size_t i=0; i<numberOfCells; i++)
		{
			...
			ccCells[i].theIndex = (	static_cast<unsigned>(cellPos.x)				)
								...
								+ (	static_cast<unsigned>(cellPos.z) << (2*level)	);//根据单元格行列号计算中心点的索引
		}
	}

    //we deduce the size of the grid that totally includes input cells
    Tuple3i gridSize = indexMax - indexMin + Tuple3i(1,1,1);//右上角点-左下角点

    //we sort the cells//根据indexComp方法进行排序
    std::sort(ccCells.begin(),ccCells.end(),IndexAndCode::indexComp); //ascending index code order

    const int& di = gridSize.x;//行数
    const int& dj = gridSize.y;//列数
    ...
    int sliceSize = (di+2)*(dj+2); //add a margin to avoid "boundary effects"  行数*列数
    ...
    //process each slice
	{
		...
		for (int k = indexMin.z; k < indexMin.z+step; k++)//高度方向遍历
		{
...
DgmOctreeNDT::octreeCellNDT::octreeCellNDT(const DgmOctreeNDT* _parentOctree)
...
DgmOctreeNDT::octreeCellNDT::octreeCellNDT(const octreeCellNDT& cell)
...
DgmOctreeNDT::octreeCellNDT::~octreeCellNDT()
{
	if (points)
		delete points;
}
...
unsigned DgmOctreeNDT::executeFunctionForAllCellsAtLevel(	unsigned char level,
														octreeCellFunc func,
														...
														int maxThreadCount/*=0*/)
{
...
#endif
	{
		...
		//for each point,m_thePointsAndTheirCellCodes几乎等同于所有点
		for (; p != m_thePointsAndTheirCellCodes.end(); ++p)
		{
			//check if it belongs to the current cell
			OctreeCellCodeType nextCode = (p->theCode >> bitDec);
			if (nextCode != cell.truncatedCode)//和当前的编码不一致
			{
...
unsigned DgmOctreeNDT::executeFunctionForAllCellsStartingAtLevel(unsigned char startingLevel,
	octreeCellFunc func,
	void** additionalParameters,
	...
	int maxThreadCount/*=0*/)
{
...
#endif
	{
		...
		unsigned char currentBitDec = GET_NDT_BIT_SHIFT(startingLevel);
...
bool DgmOctreeNDT::rayCast(const CCVector3& rayAxis,
						...
						std::vector<PointDescriptor>& output) const
{
	...
	//binary shift for cell code truncation at current level
	unsigned char currentBitDec = GET_NDT_BIT_SHIFT(level);
	//current cell code
	OctreeCellCodeType currentCode = INVALID_CELL_CODE;
	...
	for (cellsContainer::const_iterator it = m_thePointsAndTheirCellCodes.begin(); it != m_thePointsAndTheirCellCodes.end(); ++it)
	{
		...
		if (truncatedCode != (currentCode >> currentBitDec))
		{
			...
			while (level > 1)
			{
				unsigned char bitDec = GET_NDT_BIT_SHIFT(level-1);
			...
			currentBitDec = GET_NDT_BIT_SHIFT(level);
		}
