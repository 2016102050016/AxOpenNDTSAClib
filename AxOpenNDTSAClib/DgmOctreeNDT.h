...
#ifndef DGM_OCTREE_NDT_HEADER
#define DGM_OCTREE_NDT_HEADER
...
//class CCLib::ReferenceCloud;
//class CCLib::GenericIndexedCloudPersist;
...
**/
#define GET_NDT_BIT_SHIFT(level) (3*(CCNDTLib::DgmOctreeNDT::MAX_OCTREE_LEVEL-level))
...
**/
class  DgmOctreeNDT : public GenericOctree
{
public:
	...
	**/
	...
	struct CellDescriptor
	{
		...
		DgmOctreeNDT::OctreeCellCodeType Code;
	...
	**/
	...
	{
		//! index
		unsigned theIndex;//点的索引
		//! cell code
		OctreeCellCodeType theCode;//点所在八叉树单元的Code
	...
	struct octreeCellNDT
	{
		//! Octree to which the cell belongs
		const DgmOctreeNDT* parentOctree;
		...
		//! Default constructor
		explicit octreeCellNDT(const DgmOctreeNDT* parentOctree);

		//! Default destructor
		virtual ~octreeCellNDT();

	private:
		
		//! Copy constructor
		octreeCellNDT(const octreeCellNDT& cell);
	};
	...
	/** See DgmOctree::executeFunctionForAllCellsAtLevel and 
		...
		- return success
	**/
	typedef bool (*octreeCellFunc)(const octreeCellNDT& cell, void**, NormalizedProgress*);
	...
	**/
	explicit DgmOctreeNDT(GenericIndexedCloudPersist* cloud);

	//! DgmOctree destructor
	virtual ~DgmOctreeNDT();
...
public:	/***** CELLS POSITION HANDLING *****/
	...
	**/
	...
	/** The octree instance on which is "applied" this method should be the compared cloud's one.
		...
		\return the 'best' level
	**/
	unsigned char findBestLevelForComparisonWithOctree(const DgmOctreeNDT* theOtherOctree) const;
	...
	**/
	...
	//获得每层的maxCellPopulation数目
	inline const unsigned& getmaxCellPopulation(unsigned char level) const
	{
		assert(level <= MAX_OCTREE_LEVEL);
		return m_maxCellPopulation[level];
	}
